...
export const NewUserSchema = z.object({
    ...
    id: z.string().regex(/^[0-9]{9}$/, { message: 'מספר תעודת זהות יכול להכיל 9 ספרות בלבד' })
                    .refine(idNumberValidator,{
                        message: 'ישנה טעות במספר טעות הזהות'
                    }),
    birthDate: z.coerce.date().min(new Date("1900-01-01"), { message: "נראה כי אתה קצת יותר מידי מבוגר" })
                                .max(new Date(), { message: "כנראה שעוד לא נולדת" }),
    phone: z.string().regex(/^05\d{8}$/, { message: 'מספר טלפון נייד חייב להכיל 10 ספרות ולהתחיל בקידומת 05' }),
    ...
    agree: z.literal(true, {
        errorMap: () => ({ message: "על מנת להירשם עליך להסכים לתנאי השירות" }),
      }),
})
